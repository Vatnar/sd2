...
int main() {
  ...
  Arena *frame_arena =
      arena_alloc({.flags = ArenaFlags::NO_CHAIN,
                   .name  = "Frame arena"});
...
#endif
  {
...
#ifdef SD2_DEBUG
    {
      ...
      if (func)
        func(
            static_cast<VkInstance>(vk_instance), &debug_create_info, nullptr, &debug_messenger);
    }
#endif

    VkSurfaceKHR raw_surface{};
    if (glfwCreateWindowSurface(vk_instance, window.glfw_window, nullptr, &raw_surface) !=
        VK_SUCCESS) {
      TRAP();
    }
  ...
  Array<SwapchainResources, MAX_GENERATIONS>          swapchain_pool{};
  U32                                                 pool_gen{};
  ...
  vk::CommandPool                                     command_pool{};
  {
    ...
    abort_if_vk_error(vk_device.getSwapchainImagesKHR(
        vk_swapchain, &sc->image_count, sc->images));
    ...
    for (U32 i = 0; i < sc->image_count; ++i) {
      sc->render_finished_sems[i] =
          abort_if_vk_error(vk_device.createSemaphore({}));
    }
  ...
  vk::Pipeline       graphics_pipeline{};
  {
    ...
    vk::PipelineShaderStageCreateInfo vert_shader_stage_info{
        .stage  = vk::ShaderStageFlagBits::eVertex, .module = shader_module, .pName = "vert_main"};
    vk::PipelineShaderStageCreateInfo frag_shader_stage_info{
        .stage = vk::ShaderStageFlagBits::eFragment, .module = shader_module, .pName = "frag_main"};
    ...
    vk::DescriptorSetLayout descriptor_set_layout =
        abort_if_vk_error(vk_device.createDescriptorSetLayout(dslci));

    PROFILE_START("pipeline_creation");
    vk::PipelineLayoutCreateInfo pipeline_layout_info{
        .setLayoutCount = 1, .pSetLayouts = &descriptor_set_layout, .pushConstantRangeCount = 0};
    ...
    graphics_pipeline = abort_if_vk_error(vk_device.createGraphicsPipeline(
  ...
  {
    {
      ...
      auto [staging_buffer, staging_buffer_memory] = create_buffer(
          vk_phys_dev, vk_device, buffer_size,
      ...
      copy_buffer(
          vk_device, command_pool, graphics_queue, staging_buffer, vertex_buffer, buffer_size);
    ...
    }
    {
      ...
      std::tie(index_buffer, index_buffer_memory) = create_buffer(
          ...
          vk::MemoryPropertyFlagBits::eDeviceLocal);
      copy_buffer(
          vk_device, command_pool, graphics_queue, staging_buffer, index_buffer, buffer_size);
      vk_device.destroyBuffer(staging_buffer);
      vk_device.freeMemory(staging_buffer_memory);
    }
  }
  ...
  Array<vk::DescriptorSet, MAX_FRAMES_IN_FLIGHT> descriptor_sets{};
  {
    for (U64 i = 0; i < MAX_FRAMES_IN_FLIGHT; ++i) {
      ...
      uniform_buffers_mapped[i] =
          abort_if_vk_error(vk_device.mapMemory(uniform_buffers_memory[i], 0, buffer_size));
    }
    ...
    vk::DescriptorSetLayoutBinding ubo_layout_binding{
        ...
        .descriptorCount = 1, .stageFlags = vk::ShaderStageFlagBits::eVertex};
    vk::DescriptorSetLayoutCreateInfo dslci{.bindingCount = 1, .pBindings = &ubo_layout_binding};
    vk::DescriptorSetLayout temp_layout =
        abort_if_vk_error(vk_device.createDescriptorSetLayout(dslci));
    ...
    for (U64 i = 0; i < MAX_FRAMES_IN_FLIGHT; i++) {
      vk::DescriptorBufferInfo buffer_info{.buffer = uniform_buffers[i], .offset = 0,
    ...
    while (!glfwWindowShouldClose(window.glfw_window)) {
      ...
      while (window.events.pop(event)) {
        switch (event.type) {
          ...
          case NONE:     { break; }
          case RESIZE:   { window.framebuffer_resized = true; break; }
          case KEY: {
            WKeyEvent &key = event.key;
            printf("%d, %d, %d, %d\n", key.key, key.action, key.mods, key.scancode);
            if (key.action == GLFW_PRESS) {
              if (key.key == GLFW_KEY_R) clear_color = {{{0.15f, 0.05f, 0.05f, 1.0f}}};
              if (key.key == GLFW_KEY_G) clear_color = {{{0.05f, 0.15f, 0.05f, 1.0f}}};
              if (key.key == GLFW_KEY_B) clear_color = {{{0.05f, 0.05f, 0.15f, 1.0f}}};
            }
            break;
          }
          case SCROLL:   { break; }
          ...
          case MBUTTON: {
            ...
            switch (mbutton.button) {
              default: { printf("other mouse button: %d\n", mbutton.button); break; }
              case 0:  { printf("left click %s\n", action); break; }
              case 1:  { printf("right click %s\n", action); break; }
              case 2:  { printf("middle click %s\n", action); break; }
            }
            break;
          }
          case CLOSE:    { break; }
      ...
      VkResult acquire_res = vkAcquireNextImageKHR(
          vk_device, vk_swapchain, UINT64_MAX, VK_NULL_HANDLE,
          acquire_fences[current_frame], &image_index);

      if (acquire_res == VK_ERROR_OUT_OF_DATE_KHR) {
        pool_gen = (pool_gen + 1) % MAX_GENERATIONS;
        sc       = &swapchain_pool[pool_gen];
        recreate_swapchain(window.glfw_window, vk_device, sc, vk_swapchain,
                           vk_phys_dev, vk_surface, swapchain_extent,
                           chosen_format, chosen_color_space, chosen_present_mode);
        continue;
      }
      ...
      vk::ImageMemoryBarrier2 to_render_barrier = make_image_barrier(
          image_index, sc, vk::ImageLayout::eUndefined,
      ...
      vk::RenderingInfo rendering_info{
          .renderArea = vk::Rect2D{{0, 0}, swapchain_extent}, .layerCount = 1,
          .colorAttachmentCount = 1, .pColorAttachments = &color_attachment};
      ...
      cmd.bindDescriptorSets(vk::PipelineBindPoint::eGraphics, pipeline_layout, 0,
                             descriptor_sets[current_frame], nullptr);
      cmd.drawIndexed(INDICES.size(), 1, 0, 0, 0);
      cmd.endRendering();

      vk::ImageMemoryBarrier2 to_present_barrier = make_image_barrier(
          image_index, sc, vk::ImageLayout::eColorAttachmentOptimal,
      ...
      if (s_debug_accum_ms >= 1000.0) {
        ...
        printf("Frames: %-4u | work: %6.3f ms | total: %6.3f ms | wait: %6.3f ms | FPS: %6.1f\n",
               s_debug_frames, s_debug_work_sum / s_debug_frames,
               s_debug_accum_ms / s_debug_frames,
               s_debug_accum_ms / s_debug_frames - s_debug_work_sum / s_debug_frames, fps);
        s_debug_accum_ms = 0;
        s_debug_frames   = 0;
        s_debug_work_sum = 0;
      }
  ...
  for (U32 i = 0; i < MAX_GENERATIONS; ++i) {
    ...
    for (U32 j = 0; j < gen->image_count; ++j) {
      if (gen->image_views[j])        vk_device.destroyImageView(gen->image_views[j]);
      if (gen->render_finished_sems[j]) vk_device.destroySemaphore(gen->render_finished_sems[j]);
    }
  }
  for (U64 i = 0; i < MAX_FRAMES_IN_FLIGHT; ++i) {
  ...
  abort_if_vk_error(
      vk_device.freeDescriptorSets(descriptor_pool, descriptor_sets.size(), descriptor_sets));
